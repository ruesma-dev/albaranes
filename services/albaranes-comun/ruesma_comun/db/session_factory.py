# ruesma_comun/db/session_factory.py
...
class SessionFactoryDesdeEngine:
    """Variante mínima duck-type compatible (``create_session``/``engine``)
    construida sobre un ``Engine`` ya existente. Útil para tests (SQLite
    en memoria) y para servicios que ya gestionan su propio engine.
    """
