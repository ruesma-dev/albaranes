...
"""F-039 · R3–R7: las filas del informe que dependen del reloj.

Dos campañas equivalentes escriben el mismo informe salvo en los tiempos. Qué
filas son «tiempo» estaba escrito a mano dentro de un test (F-012) y nadie lo
extendió cuando F-038 añadió una fila más: el test quedó flaky. Aquí se declara
una sola vez, junto a quien escribe el informe, y se comprueba de la única
...
RUTA_TEST_F012 = Path("tests/test_f012_r1_r5_r11_coordinador.py")
...
def test_f039_r3_filas_de_reloj_es_una_tupla_de_prefijos_no_vacia() -> None:
...
def test_f039_r3_lineas_comparables_descarta_las_filas_declaradas(
    tmp_path: Path,
) -> None:
...
def test_f039_r4_dos_informes_que_solo_difieren_en_tiempos_son_comparables_iguales(
    tmp_path: Path,
) -> None:
    ...
    estas dos listas dejen de coincidir, que es exactamente el flake de F-038 T5.
    """
    rapido = _escribir(
...
def test_f039_r5_todo_prefijo_declarado_sigue_apareciendo_en_el_informe(
    tmp_path: Path,
) -> None:
    ...
    test de R4 se vuelve flaky otra vez. Esto lo caza en el acto.
    """
...
def test_f039_r5_una_fila_de_reloj_sin_declarar_rompe_la_comparacion() -> None:
    """La guarda de R5, demostrada: sin declarar, la fila nueva se ve.

    Se simula la fila que alguien añade al informe (la de F-038 T5, antes de
    declararla) y se comprueba que `lineas_comparables` NO la esconde.
    """
...
def test_f039_r6_las_diferencias_que_no_son_reloj_siguen_apareciendo(
    tmp_path: Path,
) -> None:
    ...
    variantes = {
        ...
        "sha": _informe(sha_head="ffffffffffffffffffffffffffffffffffffffff"),
...
# --- R7: el test de paridad de F-012 usa la constante, no una lista propia --


def test_f039_r7_el_test_de_paridad_de_f012_usa_lineas_comparables() -> None:
    fuente = RUTA_TEST_F012.read_text(encoding="utf-8")

    assert "lineas_comparables" in fuente, (
        "el test de paridad de F-012 debe comparar con `lineas_comparables`"
    )
    assert "def _comparable" not in fuente, (
        "sigue habiendo un filtro de filas de reloj propio en el test de F-012"
    )
    for prefijo in FILAS_DE_RELOJ:
        assert prefijo not in fuente, (
            f"el test de F-012 vuelve a declarar el prefijo {prefijo!r} por su cuenta"
        )
