...
// ------------------------------------------------------------------- //
// DESHACER por albarán (jul 2026): pill flotante SOLO en el detalle.
// El widget lleva el document_id en data-document-id y lo manda en
// cada llamada: el servidor lista/deshace únicamente lo hecho en ESE
// albarán (lo de otro albarán se deshace entrando en el suyo). Ids
...
(function wireUndoGlobal() {
    ...
    if (!widget) return;
    var docId = widget.getAttribute("data-document-id") || "";
    if (!docId) return; // sin albarán no hay historial que enseñar
    var qs = "?document_id=" + encodeURIComponent(docId);
    ...
    function refrescar() {
        fetch("/api/undo/list" + qs, { headers: { Accept: "application/json" } })
            .then(function (r) { return r.json(); })
            .then(function (data) {
...
// ------------------------------------------------------------------- //
// BORRADO EN BLOQUE (jul 2026): casillas por fila en la bandeja +
// casilla "todos" de cabecera + boton con contador en la fila de
// filtros. Al confirmar, rellena el formulario oculto con los ids
// (CSV) y lo envia: el servidor hace soft-delete (papelera) y
// redirige con el mensaje. Solo existe en la vista de activos.
// ------------------------------------------------------------------- //
(function wireBulkDelete() {
    var form = document.getElementById("bulkDeleteForm");
    var btn = document.getElementById("bulkDeleteBtn");
    if (!form || !btn) return;
    var all = document.getElementById("selAllDocs");
    function cajas() {
        return [].slice.call(document.querySelectorAll(".sel-doc"));
    }
    function marcadas() {
    ...
    function refrescar() {
        ...
        btn.textContent = "\uD83D\uDDD1 " + n;
        btn.title = n
            ? "Mover " + n + " albar\u00e1n(es) a la papelera"
            : "Marca alg\u00fan albar\u00e1n para borrar en bloque";
        if (all) {
    ...
    document.addEventListener("change", function (ev) {
        if (ev.target && ev.target.classList &&
            ev.target.classList.contains("sel-doc")) { refrescar(); }
    });
    btn.addEventListener("click", function () {
        var sel = marcadas();
        if (!sel.length) return;
        var ok = window.confirm(
            "\u00bfMover " + sel.length + " albar\u00e1n(es) a la " +
            "papelera? Podr\u00e1s restaurarlos despu\u00e9s desde la papelera."
        );
        if (!ok) return;
        form.querySelector("input[name=ids]").value = sel
