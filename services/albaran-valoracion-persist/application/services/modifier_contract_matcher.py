...
class ModifierContractMatcher:
    ...
    def _build_predicate(
        ...
        if rol == "incremento_consistencia":
            # (jul 2026) Distingue la consistencia concreta cuando la
            # sintética la nombra (FLUIDA / LÍQUIDA / SECA — la SECA
            # también tarifa, feedback JO). Así un contrato que tarife
            # varias consistencias casa con la correcta. Fallback:
            # basta CONSISTENCIA (contratos con una sola tarifa).
            if "LIQUIDA" in desc:
                return lambda d: "CONSISTENCIA" in d and "LIQUIDA" in d
            if "SECA" in desc:
                return lambda d: "CONSISTENCIA" in d and "SECA" in d
            if "FLUIDA" in desc:
                return lambda d: "CONSISTENCIA" in d and "FLUIDA" in d
        ...
        if rol == "incremento_fratasado":
            # (jul 2026) Hormigón FRATASADO — extra /F/ tras la
            # exposición (HA-25/B/12/XC2/F/P). FRATASAD cubre
            # FRATASADO/FRATASADA/FRATASADOS.
            return lambda d: "FRATASAD" in d

        if rol == "incremento_cemento":
            # (jul 2026) Cemento especial sulforresistente. Exige
            # CEMENTO + (SULFORRES* o la palabra completa SR) para no
            # casar por siglas incrustadas en otras palabras.
            return lambda d: "CEMENTO" in d and (
                "SULFORRES" in d
                or re.search(r"\bSR\b", d) is not None
            )
