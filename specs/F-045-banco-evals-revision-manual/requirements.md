...
decide (partida, precio, contrato, obra deducida) va al de la fase que lo decide
(IA3/IA4) y a `RESULTADO_FINAL`.
...
R6. El sistema debe traducir la familia escrita por el humano a una del catálogo
único (`ruesma_comun.contratos.familias`) y a su pestaña, con un fichero de
datos versionado. Etiqueta ground truth humano: NO es clasificación de
producción (§14).

R7. El sistema debe dar a cada albarán un `caso_id` estable entre pasadas, con
el prefijo de su pestaña (`GEN/HOR/MOR/RES/BOM/COM/ALQ-NNN`), y guardar en un
mapa versionado `caso_id` ↔ código de albarán ↔ nombre de fichero original ↔
formato ↔ `gemelo_de`: es lo que permite volver al papel cuando un eval falle.
...
llevan valor, cuántas `?` y cuántas vacías; el reparto entre **no regresión** y
**defecto conocido**; y las filas descartadas con su motivo (R21).
...
R20. El sistema debe sacar el código de albarán del nombre del fichero con el
convenio del humano —si el nombre sin extensión trae `_`, lo de después del
último `_`; si no, el nombre entero—, admitir `.pdf`, `.png`, `.jpg` y `.jpeg` y
emitir el plan de copia a `evals/inputs/albaranes/<caso_id><extensión>`. Manda
el código del papel, NUNCA el persistido: el precedente es `SS-0801977` leído
donde el papel decía `SS-0001977`.
...
caso queda fuera del plan y el informe lo lista uno a uno: fallo ruidoso, nunca
silencioso (importar 57 de 59 sin decir cuáles faltan es peor que no importar).

R22. CUANDO el mismo albarán exista en PDF y en imagen, los dos nombres
resuelven al MISMO código y el sistema debe crear DOS casos, `<caso_id>` y
`<caso_id>-IMG`, con el MISMO ground truth y distinta entrada, hermanados por
`gemelo_de`. No es un choque de duplicados: es el par que aísla el formato.
...
hacen IA3 e IA4, de modo que `caso_id`, `fichero_albaran` y `comentario` dejen
de contar como discrepancia.

R26. El sistema debe declarar en el informe de la pasada los campos de IA1 e IA2
que hoy no son observables (p. ej. `unidad`, pendiente de F-024), en vez de
...
patrón (id, título, fase, casos, decisión candidata, estado) más la lista de
casos de **no regresión**, los que hoy deben salir en VERDE.
...
proveedor nuevo? SI alguna es «no», ENTONCES queda `descartada` y no genera
ficha. Estrechar el espacio de búsqueda (la partida entre las de la obra; la
obra entre las del proveedor) pasa el filtro; una regla atada a un formato o a
un proveedor, no.
