...
Prerrequisito: preguntas abiertas P1–P4 de design.md respondidas por el
humano (P5 puede quedar para después).
...
- [ ] T1: Crear `services/albaran-valoracion-persist/tests/` con
  ...
  sin_senal (R1), default retirada con material (R2), contenedores
  facturables con retiradas (R6), `buscar_linea_canon` y `cantidad_canon`
  (R9). Sin red ni BBDD.
  | Verificación: `cd services/albaran-valoracion-persist && python -m pytest tests/test_f006_residuos_pago.py -q` (tests `test_f006_r1_*`, `test_f006_r2_*`, `test_f006_r6_*`, `test_f006_r9_*`)

- [ ] T2: Columna `no_facturable` en sv6: `valuation_records.py` (campo,
  default False, comentario-enum con `canon_vertedero`),
  ...
  | Verificación: `python -m pytest tests/test_f006_no_facturable.py -q` (test `test_f006_r5_*`)

- [ ] T3: Builder sv6 — lógica de pago en `_build_from_albaran_line`:
  LLEVAR → `no_facturable=True` + importe 0 + razón + sin
  `movimiento_asumido_1` (R3, R4); override retiradas (R6); resto de
  familias intactas (R14); documento todo-LLEVAR → total 0 sin canon (R15).
  Fixtures de envelope residuos (llevar / retirar / ambos) y de hormigón
  (regresión).
...
- [ ] T4: Builder sv6 — red determinista de canon
  `_sinteticas_canon_faltantes`: sintética `canon_vertedero` por retirada
  (R7), partida del recurso casado (R8), cantidad por unidad del canon con
  `canon_sin_magnitud` (R9), `canon_vertedero_no_encontrado` a revisión
  (R10), dedupe con IA3 (R11).
...
- [ ] T6: sv5 — prompt `valuation_residuos` en `config/prompts.yaml`:
  lógica de pago + canon como única sintética permitida (R12). Test que
  carga el YAML real y comprueba que `valuation_residuos` menciona
  `canon_vertedero`, la regla solo-LLEVAR y mantiene la prohibición del
  resto de sintéticas.
  | Verificación: `python -m pytest tests/test_f006_prompt_residuos.py -q` (test `test_f006_r12_*`)

- [ ] T7: Puerta F-011 — actualizar el ground truth de evals de
...
- [ ] T8: Documentación en el mismo trabajo: §9.4 y §10.6 de
  `docs/referencia/dominio_negocio_albaranes.md` (🔶 de pago → ✅, aclarar
  prioridad 2 según P2/P3 aprobadas).
