...
registros se montan en el orden de `CAMPOS`— y el 8 pasaba por alto que la
comprobación previa guardaría el libro ANTES de la copia, incumpliendo R16.
Cerradas con test, y con ellas el 9: **ante la duda, test**. La mutación
encontró además un fallo en un test MÍO —el de las copias contaba NOMBRES, y la
copia lleva la hora con precisión de minuto—.
...
banco. Dos causas, dos respuestas.
...
con un test que ata las dos listas para que no diverjan.
...
`progress/impl_F-045_contrato_lineas.md`. El informe además anunciaba los no
observables como «campos sin clasificar en `criticidad.json`» —el runner los
colaba por ese parámetro—: ahora son dos secciones, que son dos problemas.
...
corregida** y deja de ser una desviación. El incremento por LER pasa a sintética
de IA3 con una regla declarada en `vocabulario.json`, y para que las filas
viejas de IA1 no se quedaran ahí hizo falta `evals/huella_importacion.json`: las
CLAVES de lo que el importador escribió la vez anterior, que es lo único que él
puede retirar. La huella tuvo a su vez un agujero que **costó el
`modifier_source` de tres casos RES** antes de verse, ya cerrado y restaurado.
El relato entero, en `progress/impl_F-045_contrato_lineas.md`.
