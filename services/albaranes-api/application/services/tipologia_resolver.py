...
def _hay_ler_en_linea(linea: dict) -> bool:
    ...
    # (jul 2026) El codigo_ler del contexto también se VALIDA contra el
    # catálogo: la IA puede colar una referencia de producto ahí.
    if isinstance(ctx, dict) and es_ler_valido(ctx.get("codigo_ler")):
        return True
    # Red de seguridad sobre el TEXTO COMPLETO de la línea (código +
    # concepto juntos: así la palabra "LER"/"residuo" del concepto da
    # contexto a un número pegado escrito en el campo código). Ver
    # texto_contiene_ler: exige catálogo + (separadores o contexto).
    texto = f"{linea.get('codigo') or ''} {linea.get('concepto') or ''}"
