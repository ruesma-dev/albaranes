...
- **Suministro no destinado**: línea base `from_albaran` cuyo
  ...
  «Genérico / Suministros», §9.1). Ver Pregunta abierta P1 del design.
- **Literal ALM**: valor de `ALM_CODIGO_PARTIDA` (config sv6 ya existente,
  por defecto `"ALM"`).
...
- **R2.** CUANDO sv6 valora una línea base `from_albaran` SIN
  ...
  sistema debe resolver `codigo_partida_final = <literal ALM>` con el nuevo
  `partida_action = "alm_default"` y añadir la razón auditable
  `"alm_default_suministro_no_destinado"` al resultado del matcher.
...
- **R4.** CUANDO una línea de suministro no destinado sin partida impresa
  ...
  respaldo (`origen = "nueva_no_match"`) con
  `codigo_partida = <literal ALM>` (no `None`, que hoy es lo que
  arrastraría del albarán).
...
- **R6.** El sistema debe hacer que las líneas complementarias
  ...
  línea base también cuando la base resolvió `alm_default` (es decir,
  heredan el literal ALM con
...
- **R9.** CUANDO el documento tiene `obra_codigo`, existe memoria para
  ...
  (partida vacía o igual al literal ALM), el detalle del documento debe
  exponer esa partida como `partida_sugerida` y el combo debe
  prerrellenarla como sugerencia visualmente marcada; el sistema NO debe
  persistirla sin acción humana explícita de guardado (la elección es
  humana, §10.8).
- **R10.** CUANDO el revisor guarda una línea salmón con una partida real
  elegida (no vacía y distinta del literal ALM) — vía edición del campo
  partida o vía selección de línea de contrato con partida —, el sistema
  debe upsertar `partida_memoria(obra_codigo, producto_clave) →
  codigo_partida`, con instante UTC y autor si se conoce.
...
| R4 | `test_f007_r4_*` unit del fallback `nueva_no_match` del builder |
...
| R10 | `test_f007_r10_*` unit de la decisión de memorizar (qué se memoriza y qué no) con repositorio fake |
