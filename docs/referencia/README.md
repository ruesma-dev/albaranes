...
documentación del ecosistema (`azure-apps/`) como única copia, y en este
directorio queda solo un **puntero** de pocas líneas con la ruta, el motivo y
lo imprescindible para no tener que abrir el otro repositorio. Dos copias del
mismo documento divergen siempre; ya pasó con `sigrid_api.md`. El puntero
evita además arrastrar ficheros de megas a cada proyecto.
