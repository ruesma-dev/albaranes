...
- [ ] T1: Crear `evals/revision/vocabulario.json` y `vocabulario.py`: mapeo etiqueta del Excel → familia de DOCUMENTO del catálogo → pestaña (design §5 bis, con CONTENEDORES/GASOLEO/CAMION GRUA pendientes del humano), orígenes de línea y de precio con sus sinónimos, y aborto si algo no se reconoce —nunca caer en `generico`—.  |  Verificación: `pytest tests/test_f045_r5_r6_vocabulario.py` (R5, R6); traza RED en el informe
...
- [ ] T7 bis: Aplicar los tres criterios de residuos al ground truth (canon por código LER cuando el contrato no tarifa ese LER, cantidad = peso en tn con mínimo 1, incremento por año también en residuos); los tres nacen como defecto conocido porque hoy no hay código.  |  Verificación: `pytest tests/test_f045_r12bis_residuos.py` (R12 bis)
...
- [ ] T19: Validar con el humano: la tabla de reparto (§3), la política de vacíos por columna, las tres filas pendientes del mapa de etiquetas (§5 bis), si el mínimo de 1 tn alcanza al movimiento de contenedor o solo al canon (§5 ter), y el campo `servicios` (D6).  |  Verificación: MANUAL (humano) (R31)
